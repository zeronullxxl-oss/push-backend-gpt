...
function isBlacklisted(ip) {
  const row = db.prepare('SELECT 1 FROM blacklisted_ips WHERE ip = ?').get(ip);
  return !!row;
}

function getBlacklistedDeviceIds() {
  // Get device_ids associated with blacklisted IPs
  const ips = db.prepare('SELECT ip FROM blacklisted_ips').all().map(r => r.ip);
  if (!ips.length) return [];
  const deviceIds = new Set();
  for (const ip of ips) {
    const rows = db.prepare("SELECT DISTINCT device_id FROM events WHERE json_extract(data, '$.ip') = ?").all(ip);
