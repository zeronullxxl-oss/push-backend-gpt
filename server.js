          ...
          setTimeout(async () => {
            try {
              ...
              if (sub) {
                ...
                await webPush.sendNotification(pushData, JSON.stringify({ title: pi.title, body: pi.body, image: pi.image || '', url: pi.url || '/platform.html' }));
...
app.post('/api/send', async (req, res) => {
  ...
  for (const sub of subs) {
    try {
      const pushData = JSON.parse(sub.data);
      await webPush.sendNotification(pushData, JSON.stringify({ title, body, image: image || '', url: url || '/platform.html' }));
      sent++;
    } catch (e) {
      errors++;
...
async function runScheduledPushes() {
  ...
  for (const sp of due) {
    ...
    for (const sub of subs) {
      try {
        const pushData = JSON.parse(sub.data);
        await webPush.sendNotification(pushData, JSON.stringify({ title: sp.title, body: sp.body, image: sp.image || '', url: sp.url || '/platform.html' }));
        sent++;
      } catch (e) {
        errors++;
