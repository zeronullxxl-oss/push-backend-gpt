          ...
          setTimeout(async () => {
            try {
              ...
              if (sub) {
                ...
                await webPush.sendNotification(pushData, JSON.stringify({ title: pi.title, body: pi.body, image: pi.image || '', url: pi.url || '/' }));
...
app.post('/api/scheduled', (req, res) => {
  try {
    ...
    const r = db.prepare(`INSERT INTO scheduled_pushes (title, body, image, url, schedule_type, send_at, interval_minutes, next_send_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(title, body, image || '', url || '/', schedule_type || 'once', send_at || '', interval_minutes || 0, nextSend);
...
app.get('/api/push-config', (req, res) => {
  ...
  res.json(row ? JSON.parse(row.value) : { active: false, title: '', body: '', image: '', url: '/', delay: 5 });
});

app.post('/api/push-config', (req, res) => {
  const { active, title, body, image, url, delay } = req.body;
  const val = JSON.stringify({ active: !!active, title: title || '', body: body || '', image: image || '', url: url || '/', delay: delay || 5 });
...
async function runScheduledPushes() {
  ...
  for (const sp of due) {
    ...
    for (const sub of subs) {
      try {
        const pushData = JSON.parse(sub.data);
        await webPush.sendNotification(pushData, JSON.stringify({ title: sp.title, body: sp.body, image: sp.image || '', url: sp.url || '/' }));
        sent++;
      } catch (e) {
        errors++;
