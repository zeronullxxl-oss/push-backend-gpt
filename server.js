...
try { db.exec("ALTER TABLE subs ADD COLUMN device_id TEXT DEFAULT ''"); } catch(e) {}
try { db.exec("ALTER TABLE subs ADD COLUMN ip TEXT DEFAULT ''"); } catch(e) {}
try { db.exec("ALTER TABLE subs ADD COLUMN user_agent TEXT DEFAULT ''"); } catch(e) {}
try { db.exec("ALTER TABLE subs ADD COLUMN active INTEGER DEFAULT 1"); } catch(e) {}
...
app.post('/api/subscribe', (req, res) => {
  try {
    ...
    const subData = { endpoint, keys };
    db.prepare(`INSERT INTO subs (endpoint, data, device_id, ip, user_agent) VALUES (?, ?, ?, ?, ?)
      ON CONFLICT(endpoint) DO UPDATE SET data=excluded.data, device_id=excluded.device_id, ip=excluded.ip, user_agent=excluded.user_agent`)
      .run(endpoint, JSON.stringify(subData), deviceId || '', ip, ua);
    
    // Check post-install push config
    const cfg = db.prepare("SELECT value FROM push_config WHERE key = 'post_install'").get();
    if (cfg && cfg.value) {
      try {
        const pi = JSON.parse(cfg.value);
        if (pi.active && pi.title && pi.body && pi.delay >= 0) {
          setTimeout(async () => {
            try {
              const sub = db.prepare('SELECT * FROM subs WHERE endpoint = ? AND active = 1').get(endpoint);
              if (sub) {
                const pushData = JSON.parse(sub.data);
                var piUrl = (pi.url && pi.url !== '/') ? pi.url : '/platform.html';
                await webPush.sendNotification(pushData, JSON.stringify({ title: pi.title, body: pi.body, image: pi.image || '', url: piUrl }));
                db.prepare("INSERT INTO push_log (push_type, title, sent, errors) VALUES ('post_install', ?, 1, 0)").run(pi.title);
              }
            } catch(e) {}
          }, (pi.delay || 0) * 1000);
        }
...
app.post('/api/send', async (req, res) => {
  ...
  const subs = db.prepare('SELECT * FROM subs WHERE active = 1').all();
  let sent = 0, errors = 0;
  const toDelete = [];
  ...
  for (const sub of subs) {
    try {
      const pushData = JSON.parse(sub.data);
      var pushUrl = (url && url !== '/') ? url : '/platform.html';
      await webPush.sendNotification(pushData, JSON.stringify({ title, body, image: image || '', url: pushUrl }));
      sent++;
    } catch (e) {
      errors++;
  ...
  db.prepare("INSERT INTO push_log (push_type, title, sent, errors) VALUES ('manual', ?, ?, ?)").run(title, sent, errors);
...
app.get('/api/subs', (req, res) => {
  const subs = db.prepare('SELECT id, device_id, ip, user_agent, active, created_at FROM subs ORDER BY id DESC').all();
  res.json(subs);
});

app.delete('/api/subs/:id', (req, res) => {
  db.prepare('DELETE FROM subs WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});

app.patch('/api/subs/:id/toggle', (req, res) => {
  const sub = db.prepare('SELECT active FROM subs WHERE id = ?').get(req.params.id);
  if (!sub) return res.status(404).json({ error: 'not found' });
  const newActive = sub.active ? 0 : 1;
  db.prepare('UPDATE subs SET active = ? WHERE id = ?').run(newActive, req.params.id);
  res.json({ ok: true, active: newActive });
});

// Bulk delete by IP
app.post('/api/subs/delete-by-ip', (req, res) => {
  const { ip } = req.body;
  if (!ip) return res.status(400).json({ error: 'ip required' });
  const r = db.prepare('DELETE FROM subs WHERE ip = ?').run(ip);
  res.json({ ok: true, deleted: r.changes });
});

// Bulk delete by device_id
app.post('/api/subs/delete-by-device', (req, res) => {
  const { deviceId } = req.body;
  if (!deviceId) return res.status(400).json({ error: 'deviceId required' });
  const r = db.prepare('DELETE FROM subs WHERE device_id = ?').run(deviceId);
  res.json({ ok: true, deleted: r.changes });
});
...
app.get('/api/scheduled', (req, res) => {
  res.json(db.prepare('SELECT * FROM scheduled_pushes ORDER BY id DESC').all());
});

app.post('/api/scheduled', (req, res) => {
  try {
    const { title, body, image, url, schedule_type, send_at, interval_minutes } = req.body;
    if (!title || !body) return res.status(400).json({ error: 'title and body required' });
    
    let nextSend = '';
    if (schedule_type === 'once' && send_at) {
      nextSend = send_at;
    } else if (schedule_type === 'interval' && interval_minutes > 0) {
      nextSend = new Date(Date.now() + interval_minutes * 60000).toISOString();
    } else if (schedule_type === 'daily' && send_at) {
      // send_at = "HH:MM" format
      const [h, m] = send_at.split(':').map(Number);
      const now = new Date();
    ...
    const r = db.prepare(`INSERT INTO scheduled_pushes (title, body, image, url, schedule_type, send_at, interval_minutes, next_send_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(title, body, image || '', url || '/platform.html', schedule_type || 'once', send_at || '', interval_minutes || 0, nextSend);
...
app.delete('/api/scheduled/:id', (req, res) => {
  db.prepare('DELETE FROM scheduled_pushes WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});

app.patch('/api/scheduled/:id/toggle', (req, res) => {
  const sp = db.prepare('SELECT active FROM scheduled_pushes WHERE id = ?').get(req.params.id);
  if (!sp) return res.status(404).json({ error: 'not found' });
  db.prepare('UPDATE scheduled_pushes SET active = ? WHERE id = ?').run(sp.active ? 0 : 1, req.params.id);
  res.json({ ok: true, active: sp.active ? 0 : 1 });
});

// ══════════════════════════════════════
//  POST-INSTALL PUSH CONFIG
// ══════════════════════════════════════

app.get('/api/push-config', (req, res) => {
  const row = db.prepare("SELECT value FROM push_config WHERE key = 'post_install'").get();
  res.json(row ? JSON.parse(row.value) : { active: false, title: '', body: '', image: '', url: '/platform.html', delay: 5 });
});

app.post('/api/push-config', (req, res) => {
  const { active, title, body, image, url, delay } = req.body;
  const val = JSON.stringify({ active: !!active, title: title || '', body: body || '', image: image || '', url: url || '/platform.html', delay: delay || 5 });
  db.prepare("INSERT INTO push_config (key, value) VALUES ('post_install', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(val);
  res.json({ ok: true });
});

// Push log
app.get('/api/push-log', (req, res) => {
  res.json(db.prepare('SELECT * FROM push_log ORDER BY id DESC LIMIT 50').all());
});
...
async function runScheduledPushes() {
  ...
  const due = db.prepare("SELECT * FROM scheduled_pushes WHERE active = 1 AND next_send_at != '' AND next_send_at <= ?").all(now);
  ...
  for (const sp of due) {
    ...
    for (const sub of subs) {
      try {
        const pushData = JSON.parse(sub.data);
        var spUrl = (sp.url && sp.url !== '/') ? sp.url : '/platform.html';
        await webPush.sendNotification(pushData, JSON.stringify({ title: sp.title, body: sp.body, image: sp.image || '', url: spUrl }));
        sent++;
      } catch (e) {
        errors++;
        if (e.statusCode === 404 || e.statusCode === 410) toDelete.push(sub.endpoint);
      }
    }
    ...
    db.prepare("INSERT INTO push_log (push_type, title, sent, errors) VALUES ('scheduled', ?, ?, ?)").run(sp.title, sent, errors);
    
    // Update next_send_at based on schedule_type
    if (sp.schedule_type === 'once') {
      db.prepare('UPDATE scheduled_pushes SET active = 0, last_sent_at = ? WHERE id = ?').run(now, sp.id);
    } else if (sp.schedule_type === 'interval') {
      const next = new Date(Date.now() + sp.interval_minutes * 60000).toISOString();
      db.prepare('UPDATE scheduled_pushes SET last_sent_at = ?, next_send_at = ? WHERE id = ?').run(now, next, sp.id);
    } else if (sp.schedule_type === 'daily') {
      const [h, m] = sp.send_at.split(':').map(Number);
      const next = new Date();
      next.setUTCHours(h, m, 0, 0);
      next.setDate(next.getDate() + 1);
      db.prepare('UPDATE scheduled_pushes SET last_sent_at = ?, next_send_at = ? WHERE id = ?').run(now, next.toISOString(), sp.id);
    }
    
    console.log(`📨 Scheduled push "${sp.title}": sent=${sent}, errors=${errors}`);
  }
}

// Run scheduler every 30 seconds
setInterval(runScheduledPushes, 30000);
...
app.get('/api/analytics', (req, res) => {
  try {
    ...
    const overview = {
      ...
      pushSubs: db.prepare('SELECT COUNT(*) as c FROM subs WHERE active = 1').get().c,
...
app.post('/api/templates', (req, res) => {
  try {
    ...
    const r = db.prepare('INSERT INTO push_templates (name, title, body, image, url) VALUES (?, ?, ?, ?, ?)')
      .run(name, title, body, image || '', url || '/platform.html');
    res.json({ ok: true, id: r.lastInsertRowid });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
